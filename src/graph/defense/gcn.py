...
class GCN(nn.Module):
    ...
    def __init__(self, nhids, dropout=0.5, lr=0.01, weight_decay=5e-4,
        ...
        for i in range(len(nhids)-1):
            self.gc_list.append(GraphConvolution(nhids[i], nhids[i+1], with_bias=with_bias))
    ...
    def forward(self, x, adj):
        ...
        for i, gc_layer in enumerate(self.gc_list[:-1]):
            if self.with_relu:
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, **kwargs):
        ...
        self.device = self.gc_list[0].weight.device
        if initialize:
            self.initialize()
...
class GCN_Reg(nn.Module):
    ...
    def __init__(self, nhids, y_adj, dropout=0.5, lr=0.01, weight_decay=5e-4,
            with_relu=True, with_bias=True, device=None):
    ...
    def forward(self, x, adj):
        ...
        return self.dc(x), F.log_softmax(x, dim=1)
    ...
    def _train_without_val(self, labels, idx_train, train_iters, verbose):
        ...
        for i in range(train_iters):
            optimizer.zero_grad()
            estim_adj, output = self.forward(self.features, self.adj_norm)
            loss_train = F.nll_loss(output[idx_train], labels[idx_train])
