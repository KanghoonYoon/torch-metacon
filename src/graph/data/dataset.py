...
from src.graph.utils import get_train_val_test, get_train_val_test_gcn
import zipfile
import json
...
class Dataset():
    ...
    def __init__(self, root, name, setting='nettack', seed=None, split=None, require_mask=False):
    ...
    def get_train_val_test(self):
        ...
        if self.setting == 'nettack':
            return get_train_val_test(nnodes=self.adj.shape[0], val_size=self.split[1], test_size=self.split[2], stratify=self.labels, seed=self.seed)
        if self.setting == 'gcn':
...
if __name__ == '__main__':
    from deeprobusts.graph.data import Dataset
